...
Each decision should be resolved through the appropriate PRD update, ADR, issue, or product decision record before the dependent implementation becomes authoritative.
...
| OD-005 | Which social publishing adapter is first? | Manual export comes before direct publishing. Adapter priority depends on audience fit, official access, scope, review requirements, cost, and maintenance burden. | After Campaign and Canonical Asset slice | Integration assessment and ADR if architecture changes | Deferred |
| OD-006 | Which analytics and search integrations are first? | Manual import is acceptable for the first complete loop. | Before Website, Search, and Measurement implementation | Integration assessment | Open |
...
| OD-015 | What is the first persistent schema and migration policy? | Canonical identities and context ownership must be stable before implementation. | Before domain persistence implementation | ADR and schema design | Open |
...
| OD-021 | What qualifies as a verified external delivery outcome? | Provider response or explicit human-recorded evidence is required. Scheduled state or intent is insufficient. | During issue #7 | ADR extension or domain decision | Open |
| OD-022 | Which attribution models ship first? | Manual, first-touch, last-touch, and influence views may be supported, but every result must state its model and uncertainty. | Before Measurement implementation | Product decision | Deferred |
