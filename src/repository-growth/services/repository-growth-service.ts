import type { CampaignWorkspace, RepositoryMetricObservation as Never } from "../../campaigns/domain/campaign.js";
...
export class RepositoryGrowthService {
  ...
  async importRepository(workspaceId: string, outcome: RepositoryImportOutcome): Promise<RepositoryGrowthWorkspace> {
    ...
    const previous = workspace.repositories.find((item) => item.fullName.toLocaleLowerCase("en-US") === outcome.snapshot!.fullName.toLocaleLowerCase("en-US"));
  ...
  async assessRepository(workspaceId: string, repositoryId: string, owner: string): Promise<RepositoryGrowthWorkspace> {
    ...
    const findings = assess(repository, owner.trim(), () => this.createId());
    const assessment = { id: this.createId(), repositoryId, createdAt: now, createdBy: owner.trim(), findings };
  ...
  async createGrowthPlan(workspaceId: string, repositoryId: string, assessmentId: string, owner: string): Promise<RepositoryGrowthWorkspace> {
    ...
    const actionable = assessment.findings
    ...
    if (actionable.length === 0) throw new Error("The assessment has no readiness gaps requiring a growth plan");
    const now = this.clock().toISOString();
    const plan = {
      id: this.createId(), repositoryId, assessmentId, owner: owner.trim(), createdAt: now,
      actions: actionable.map((finding) => ({
        id: this.createId(), findingId: finding.id, title: finding.recommendation, owner: owner.trim(),
        impact: finding.impact, effort: finding.effort, verification: finding.verification, status: "open" as const,
      })),
    };
  ...
  async updateGrowthAction(workspaceId: string, planId: string, actionId: string, status: RepositoryGrowthActionStatus): Promise<RepositoryGrowthWorkspace> {
    const workspace = await this.load(workspaceId);
    if (!workspace.plans.some((item) => item.id === planId)) throw new Error("Repository growth plan not found");
    let changed = false;
    const plans = workspace.plans.map((plan) => plan.id !== planId ? plan : {
      ...plan,
  ...
  async createLaunchRoom(workspaceId: string, input: Readonly<{
    ...
    for (const [value, label] of [
      [input.title, "Launch room title"], [input.primaryAudience, "Primary audience"], [input.desiredOutcome, "Desired outcome"],
      [input.releaseTag, "Release tag"], [input.createdBy, "Launch room creator"],
    ...
    const campaigns = await this.requiredCampaignWorkspace(workspaceId);
    const authority = approvedAuthority(campaigns, input.campaignId, input.canonicalAssetId);
    ...
    const now = this.clock().toISOString();
    const checklist: LaunchChecklistItem[] = clean(input.checklist).map((label) => ({ id: this.createId(), label, required: true, complete: false }));
    const room: RepositoryLaunchRoom = {
      id: this.createId(), repositoryId: repository.id, title: input.title.trim(), primaryAudience: input.primaryAudience.trim(),
      desiredOutcome: input.desiredOutcome.trim(), releaseTag: input.releaseTag.trim(), campaignId: authority.campaign.id,
      canonicalAssetId: authority.asset.id, variantIds: authority.variants.map((item) => item.id), checklist,
      maintainerCoverage: input.maintainerCoverage.map((item) => ({ ...item, owner: item.owner.trim(), responsibility: item.responsibility.trim() })),
      observationStartsAt: input.observationStartsAt, observationEndsAt: input.observationEndsAt,
      retrospectiveAt: input.retrospectiveAt, baseline: repository.metrics.map((item) => ({ ...item })), status: "draft",
      createdAt: now, createdBy: input.createdBy.trim(),
    };
  ...
  async setChecklistItem(workspaceId: string, launchRoomId: string, itemId: string, complete: boolean, evidence?: string): Promise<RepositoryGrowthWorkspace> {
    const workspace = await this.load(workspaceId);
    let changed = false;
    const rooms = workspace.launchRooms.map((room) => {
      if (room.id !== launchRoomId) return room;
      const checklist = room.checklist.map((item) => {
        if (item.id !== itemId) return item;
        changed = true;
        return { ...item, complete, ...(evidence?.trim() ? { evidence: evidence.trim() } : {}) };
      });
      const ready = checklist.every((item) => !item.required || item.complete);
      return { ...room, checklist, status: ready ? "ready_for_manual_launch" as const : "draft" as const };
    });
    if (!changed) throw new Error("Launch checklist item not found");
    return this.persist({ ...workspace, launchRooms: rooms, updatedAt: this.clock().toISOString() });
  }
  ...
  async createManualExport(workspaceId: string, launchRoomId: string, createdBy: string): Promise<RepositoryGrowthWorkspace> {
    ...
    const campaigns = await this.requiredCampaignWorkspace(workspaceId);
    const authority = approvedAuthority(campaigns, room.campaignId, room.canonicalAssetId);
    ...
    const now = this.clock().toISOString();
    const record = {
      id: this.createId(), launchRoomId: room.id, repositoryId: repository.id, createdAt: now, createdBy: createdBy.trim(),
      status: "manual_export_ready" as const,
      manifest: JSON.stringify({
        repository: { fullName: repository.fullName, url: repository.url, releaseTag: room.releaseTag, limitations: repository.limitations },
        launch: {
          primaryAudience: room.primaryAudience, desiredOutcome: room.desiredOutcome,
          observationStartsAt: room.observationStartsAt, observationEndsAt: room.observationEndsAt,
          checklist: room.checklist, maintainerCoverage: room.maintainerCoverage, baseline: room.baseline,
        },
        campaign: {
          id: authority.campaign.id, version: authority.campaign.version, claims: authority.campaign.claimReferences,
          evidenceIds: authority.campaign.evidenceIds, callToAction: authority.campaign.callToAction,
        },
        canonicalAsset: {
          id: authority.asset.id, version: latestVersion.version, body: latestVersion.body,
          rights: authority.asset.rights, accessibilityRequirements: authority.asset.accessibilityRequirements,
          disclosureRequirements: authority.asset.disclosureRequirements,
        },
        variants: authority.variants.map((item) => ({ id: item.id, channel: item.channel, version: item.version, body: item.body, constraints: item.constraints })),
  ...
  async completeRetrospective(workspaceId: string, launchRoomId: string, input: Readonly<{
    ...
    const retrospective = {
      id: this.createId(), launchRoomId: room.id, repositoryId: room.repositoryId, completedAt: now,
      completedBy: input.completedBy.trim(), summary: input.summary.trim(), learnings: clean(input.learnings),
      nextAction: input.nextAction.trim(), outcomes: input.outcomes.map((item) => ({ ...item })),
      comparisons: compareMetrics(room.baseline, input.outcomes),
    };
    const launchRooms = workspace.launchRooms.map((item) => item.id === room.id ? { ...item, status: "retrospective_complete" as const } : item);
    return this.persist({ ...workspace, launchRooms, retrospectives: [...workspace.retrospectives, retrospective], updatedAt: now });
  }

  private async requiredCampaignWorkspace(workspaceId: string): Promise<CampaignWorkspace> {
    const workspace = await this.campaignStore.load(workspaceId);
    if (!workspace) throw new Error("Campaign workspace not found");
    return workspace;
  }
...
function assess(repository: PublicRepositorySnapshot, owner: string, id: IdFactory): readonly RepositoryReadinessFinding[] {
  ...
  const findings: Omit<RepositoryReadinessFinding, "id" | "owner">[] = [
    finding("problem_clarity", score(Boolean(repository.description), repository.frontDoor.readmePresent, /(problem|helps|for teams|for developers|for maintainers|outcome)/.test(readme), readme.length > 500),
      ...
      "high", "small", "State the specific problem, audience, and useful outcome in the description and README opening.", "A new visitor can explain the problem, audience, and outcome after reading the repository first screen."),
    ...
    finding("differentiation", score(/alternative|compared|versus|vs\.|different|trade-?off|why /.test(readme), /architecture/.test(readme), /use case|when to use/.test(readme), /limitation|not intended|does not/.test(readme)),
      [/(alternative|compared|versus|vs\.|different)/.test(readme) ? "README includes alternative or comparison language" : "No explicit alternative framing detected", /limitation|not intended|does not/.test(readme) ? "Tradeoffs or limitations are visible" : "Tradeoffs are not visible"],
      "medium", "medium", "Explain alternatives, distinctive strengths, tradeoffs, and the situations where the repository is the right choice.", "A qualified evaluator can distinguish the project from plausible alternatives without guessing."),
    finding("trust", score(repository.community.licensePresent, repository.community.securityPresent, repository.frontDoor.readmePresent, !repository.archived),
      ...
      "high", "small", "Publish accurate license, security-reporting, support, ownership, maturity, and limitation guidance.", "A reviewer can identify reuse rights, vulnerability reporting, support boundaries, ownership, and current maturity."),
    finding("community_readiness", score(repository.community.contributingPresent, repository.community.codeOfConductPresent, repository.community.issueTemplatePresent, repository.community.pullRequestTemplatePresent),
      ...
      "medium", "medium", "Provide realistic contribution guidance, participation boundaries, and templates that collect actionable information.", "An unfamiliar contributor can identify an appropriate task and submit a useful issue or change."),
    ...
    finding("commercial_path", score(Boolean(repository.homepage), /(enterprise|commercial|services|support plan|sponsor|contact|book a|demo request)/.test(readme), /pricing/.test(readme), /case study|customer|used by/.test(readme)),
      ...
      "low", "medium", "Provide an honest path for sponsorship, services, support, partnership, or commercial evaluation when appropriate.", "A qualified organization can identify the supported business path without confusing community participation with a sales target."),
  ];
  return findings.map((item) => ({ ...item, id: id(), owner }));
}
...
function formatMetric(metric?: RepositoryMetricObservation): string {
  ...
  if (metric.state === "unavailable" || metric.state === "not_collected") return metric.state.replaceAll("_", " ");
  return String(metric.value ?? 0);
}

function approvedAuthority(workspace: CampaignWorkspace, campaignId: string, canonicalAssetId: string) {
  ...
  if (asset.status !== "approved" || asset.campaignId !== campaign.id) throw new Error("Repository launch requires the approved campaign canonical asset");
  ...
  if (!["linkedin", "website", "github_release"].every((channel) => channels.has(channel as "linkedin"))) {
...
function compareMetrics(baseline: readonly RepositoryMetricObservation[], outcomes: readonly RepositoryMetricObservation[]): readonly RepositoryMetricComparison[] {
  ...
  return kinds.map((kind) => {
    ...
    const comparable = isNumericState(baselineState) && isNumericState(outcomeState) && before?.value !== undefined && after?.value !== undefined;
    return {
      kind, baselineState, outcomeState,
...
function validateMetrics(metrics: readonly RepositoryMetricObservation[]): void {
  ...
  for (const metric of metrics) {
    ...
    if (isNumericState(metric.state)) {
      if (metric.value === undefined || !Number.isFinite(metric.value) || metric.value < 0) throw new Error(`Metric ${metric.kind} requires a non-negative numeric value`);
      if (metric.state === "verified_zero" && metric.value !== 0) throw new Error(`Metric ${metric.kind} marked verified zero must equal zero`);
...
function emptyWorkspace(workspaceId: string, updatedAt: string): RepositoryGrowthWorkspace {
  return { workspaceId, repositories: [], assessments: [], plans: [], launchRooms: [], exports: [], retrospectives: [], updatedAt };
}
