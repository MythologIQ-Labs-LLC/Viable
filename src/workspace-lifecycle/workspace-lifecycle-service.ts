...
export const WORKSPACE_CONTEXTS = [
  ...
  { name: "campaign", label: "Campaigns and exports", prefix: "viable.campaign-workspace.", identityField: "workspaceId", recordFields: ["campaigns", "contentBriefs", "assets", "variants", "exports"], requiredArrays: ["campaigns", "assets", "variants", "exports"], optionalArrays: ["contentBriefs"], requiredStrings: ["updatedAt"], requiredRecords: [], optionalRecords: [] },
  { name: "signals", label: "Signals", prefix: "viable.signals-inbox.", identityField: "workspaceId", recordFields: ["sources", "sourceHealth", "signals", "conversions"], requiredArrays: ["sources", "sourceHealth", "signals", "conversions"], optionalArrays: [], requiredStrings: ["updatedAt"], requiredRecords: [], optionalRecords: [] },
  { name: "activation", label: "Calendar and Learning", prefix: "viable.activation-learning.", identityField: "workspaceId", recordFields: ["destinations", "calendarEntries", "packages", "exportOperations", "deliveryOutcomes", "measurementPlans", "performanceImports", "retrospectives", "learningLedger"], requiredArrays: ["destinations", "calendarEntries", "packages", "exportOperations", "deliveryOutcomes", "measurementPlans", "performanceImports", "retrospectives", "learningLedger"], optionalArrays: [], requiredStrings: ["updatedAt"], requiredRecords: [], optionalRecords: [] },
  { name: "repositoryGrowth", label: "Repository Growth", prefix: "viable.repository-growth.", identityField: "workspaceId", recordFields: ["repositories", "assessments", "plans", "launchRooms", "exports", "retrospectives"], requiredArrays: ["repositories", "assessments", "plans", "launchRooms", "exports", "retrospectives"], optionalArrays: [], requiredStrings: ["updatedAt"], requiredRecords: [], optionalRecords: [] },
  { name: "videoProduction", label: "Video Production", prefix: "viable.video-production.", identityField: "workspaceId", recordFields: ["tools", "briefs", "packages", "artifacts", "variants"], requiredArrays: ["tools", "briefs", "packages", "artifacts", "variants"], optionalArrays: [], requiredStrings: ["updatedAt"], requiredRecords: [], optionalRecords: [] },
...
function assertContextShape(value: Readonly<Record<string, unknown>>, descriptor: ContextDescriptor): void {
  ...
  for (const field of descriptor.optionalArrays) {
    if (value[field] !== undefined && !Array.isArray(value[field])) throw new Error(`${descriptor.label} optional field ${field} must be an array when present`);
  }
  ...
  for (const field of descriptor.optionalRecords) {
    if (value[field] !== undefined && !isRecord(value[field])) throw new Error(`${descriptor.label} optional field ${field} must be an object when present`);
  }
}
