...
function enhanceProductTruth(workspace: ProductWorkspace): void {
  ...
  wrapper.innerHTML = `<div class="state"><strong>Revision-aware Product Truth editor</strong><span>Every modeled user-controlled field is editable here. Saving records who changed what and why, and it never rewrites prior revision history.</span></div>
    ...
    ${productTruthHistory(workspace)}`;
  legacy?.insertAdjacentElement("afterend", wrapper);
  if (!legacy) panel.append(wrapper);
}

function productTruthHistory(workspace: ProductWorkspace): string {
  const history = workspace.product.history ?? [];
  return `<details data-ux-revision-history><summary>Product Truth revision history (${history.length})</summary>${history.length ? [...history].reverse().map((revision) => `<article class="record"><strong>Revision ${revision.revision}</strong><p>${escapeHtml(revision.rationale)}</p><small>${humanDate(revision.changedAt)} · ${escapeHtml(revision.changedBy)} · Changed: ${escapeHtml(revision.changedFields.join(", "))}</small></article>`).join("") : `<p>No prior Product Truth revisions are recorded yet.</p>`}</details>`;
}
...
function icpRevisionSurface(workspace: ProductWorkspace, hypothesis: IcpHypothesis): string {
  ...
  return `<details data-ux-icp-revision><summary>Correct ICP and inspect revision history</summary>
    <p class="guidance">Editing a selected ICP does not silently replace selected authority. The corrected hypothesis returns to candidate review and must receive named review before it can be selected again.</p>
    <form data-ux-revision-form="icp" data-record-id="${escapeHtml(hypothesis.id)}"><div data-ux-revision-error aria-live="polite"></div>
      ...
      <div class="two"><label>Primary users<textarea name="users" rows="2">${escapeHtml(hypothesis.roles.users.join("\n"))}</textarea></label><label>Economic buyers<textarea name="economicBuyers" rows="2">${escapeHtml(hypothesis.roles.economicBuyers.join("\n"))}</textarea></label></div>
      <div class="two"><label>Decision makers<textarea name="decisionMakers" rows="2">${escapeHtml(hypothesis.roles.decisionMakers.join("\n"))}</textarea></label><label>Approvers<textarea name="approvers" rows="2">${escapeHtml(hypothesis.roles.approvers.join("\n"))}</textarea></label></div>
      <div class="two"><label>Influencers<textarea name="influencers" rows="2">${escapeHtml(hypothesis.roles.influencers.join("\n"))}</textarea></label><label>Champions<textarea name="champions" rows="2">${escapeHtml(hypothesis.roles.champions.join("\n"))}</textarea></label></div>
      <div class="two"><label>Blockers<textarea name="blockers" rows="2">${escapeHtml(hypothesis.roles.blockers.join("\n"))}</textarea></label><label>Partners<textarea name="partners" rows="2">${escapeHtml(hypothesis.roles.partners.join("\n"))}</textarea></label></div>
      <div class="two"><label>Maintainers<textarea name="maintainers" rows="2">${escapeHtml(hypothesis.roles.maintainers.join("\n"))}</textarea></label><label>Contributors<textarea name="contributors" rows="2">${escapeHtml(hypothesis.roles.contributors.join("\n"))}</textarea></label></div>
      ...
      <label>Reviewed evidence references</label><fieldset>${workspace.evidence.filter((item) => item.reviewStatus === "reviewed" && item.origin !== "generated_suggestion").map((item) => `<label class="choice"><input type="checkbox" name="evidenceIds" value="${escapeHtml(item.id)}" ${hypothesis.evidenceIds.includes(item.id) ? "checked" : ""}>${escapeHtml(item.title)}</label>`).join("") || `<span>No reviewed evidence is available.</span>`}</fieldset>
      ...
      <div class="dimension-list">${ICP_DIMENSIONS.map((dimension) => { const value = hypothesis.dimensions[dimension]; return `<fieldset class="dimension"><legend>${dimensionLabels[dimension]}</legend><div class="three"><label>Rating<select name="${dimension}.rating">${[0, 1, 2, 3, 4].map((rating) => `<option value="${rating}" ${value.rating === rating ? "selected" : ""}>${rating}</option>`).join("")}</select></label><label>Confidence<select name="${dimension}.confidence">${["low", "medium", "high"].map((confidence) => `<option ${value.confidence === confidence ? "selected" : ""}>${confidence}</option>`).join("")}</select></label><label>Evidence IDs<input value="${escapeHtml(value.evidenceIds.length)} linked" disabled></label></div><label>Rationale<textarea name="${dimension}.rationale" required rows="2">${escapeHtml(value.rationale)}</textarea></label></fieldset>`; }).join("")}</div>
...
function enhanceCampaigns(product: ProductWorkspace, workspace: CampaignWorkspace, nav: string): void {
  ...
  if (nav === "studio") enhanceVariantCards(workspace);
}
...
function campaignRevisionMarkup(product: ProductWorkspace, campaign: CampaignBrief): string {
  ...
  return `<details data-ux-campaign-revision><summary>Correct campaign and inspect revision history</summary>
    <p class="guidance">A correction creates a new campaign version. Approved campaign authority is invalidated until the corrected version receives named re-review. Approved descendants are invalidated when their governing campaign changes.</p>
    <form data-ux-revision-form="campaign" data-record-id="${escapeHtml(campaign.id)}"><div data-ux-revision-error aria-live="polite"></div>
      <div class="two"><label>Campaign title<input name="title" required value="${escapeHtml(campaign.title)}"></label><label>Named editor<input name="editor" required value="${escapeHtml(campaign.owner)}"></label></div>
      <label>Revision rationale<textarea name="rationale" required rows="2"></textarea></label><label>Objective<textarea name="objective" required rows="2">${escapeHtml(campaign.objective)}</textarea></label>
      <div class="two"><label>Primary outcome<input name="primaryOutcome" required value="${escapeHtml(campaign.primaryOutcome)}"></label><label>Primary audience<input name="primaryAudience" required value="${escapeHtml(campaign.primaryAudience)}"></label></div>
      <div class="two"><label>Audience authority<select name="audienceKind"><option value="test_audience" ${campaign.audienceKind === "test_audience" ? "selected" : ""}>Deliberate test audience</option><option value="selected_icp" ${campaign.audienceKind === "selected_icp" ? "selected" : ""}>Selected ICP</option></select></label><label>Selected ICP reference<select name="icpHypothesisId"><option value="">No ICP reference</option>${product.icpHypotheses.filter((item) => item.status === "selected" && item.reviewStatus === "reviewed").map((item) => `<option value="${escapeHtml(item.id)}" ${campaign.icpHypothesisId === item.id ? "selected" : ""}>${escapeHtml(item.name)}</option>`).join("")}</select></label></div>
      <div class="two"><label>Problem<textarea name="problem" required rows="2">${escapeHtml(campaign.problem)}</textarea></label><label>Trigger<textarea name="trigger" required rows="2">${escapeHtml(campaign.trigger)}</textarea></label></div>
      <div class="two"><label>Offer<textarea name="offer" required rows="2">${escapeHtml(campaign.offer)}</textarea></label><label>Call to action<textarea name="callToAction" required rows="2">${escapeHtml(campaign.callToAction)}</textarea></label></div>
      ...
      <div class="two"><label>Dependencies<textarea name="dependencies" rows="3">${escapeHtml(campaign.dependencies.join("\n"))}</textarea></label><label>Owner<input name="owner" required value="${escapeHtml(campaign.owner)}"></label></div>
      <button type="submit">Save campaign correction for re-review</button>
    </form>
    ${authorityHistory(campaign.history ?? [], "campaign")}
  </details>`;
}

function enhanceContentBriefCards(workspace: CampaignWorkspace): void {
  if (!main) return;
  const cards = main.querySelectorAll<HTMLElement>("[data-ux-content-brief]");
  cards.forEach((card) => {
    const id = card.dataset.recordId;
    const brief = workspace.contentBriefs?.find((candidate) => candidate.id === id);
...
function contentRevisionMarkup(brief: ContentBrief): string {
  return `<details data-ux-content-revision><summary>Correct content brief and inspect revision history</summary><form data-ux-revision-form="content-brief" data-record-id="${escapeHtml(brief.id)}"><div data-ux-revision-error aria-live="polite"></div>
    <div class="two"><label>Title<input name="title" required value="${escapeHtml(brief.title)}"></label><label>Owner<input name="owner" required value="${escapeHtml(brief.owner)}"></label></div><label>Named editor<input name="editor" required value="${escapeHtml(brief.owner)}"></label><label>Revision rationale<textarea name="rationale" required rows="2"></textarea></label>
    <label>Objective<textarea name="objective" required rows="2">${escapeHtml(brief.objective)}</textarea></label><div class="two"><label>Pillars<textarea name="pillars" required rows="3">${escapeHtml(brief.pillars.join("\n"))}</textarea></label><label>Themes<textarea name="themes" rows="3">${escapeHtml(brief.themes.join("\n"))}</textarea></label></div><div class="two"><label>Deliverables<textarea name="deliverables" required rows="3">${escapeHtml(brief.deliverables.join("\n"))}</textarea></label><label>Source notes<textarea name="sourceNotes" rows="3">${escapeHtml(brief.sourceNotes.join("\n"))}</textarea></label></div><button type="submit">Save content brief correction for re-review</button></form>${authorityHistory(brief.history ?? [], "content brief")}</details>`;
}

function enhanceVariantCards(workspace: CampaignWorkspace): void {
  if (!main) return;
  const heading = main.querySelector("#variant-heading");
  const panel = heading?.closest("section.panel");
  if (!panel) return;
  const cards = panel.querySelectorAll<HTMLElement>(".cards > article.record");
  cards.forEach((card, index) => {
    ...
    card.insertAdjacentHTML("beforeend", `<details data-ux-variant-revision><summary>Correct channel variant and inspect revision history</summary><form data-ux-revision-form="variant" data-record-id="${escapeHtml(variant.id)}"><div data-ux-revision-error aria-live="polite"></div><label>Named editor<input name="editor" required></label><label>Revision rationale<textarea name="rationale" required rows="2"></textarea></label><label>Channel body<textarea name="body" required rows="6">${escapeHtml(variant.body)}</textarea></label><label>Constraints<textarea name="constraints" required rows="3">${escapeHtml(variant.constraints.join("\n"))}</textarea></label><button type="submit">Save channel correction for re-review</button></form>${authorityHistory(variant.history ?? [], "channel variant")}</details>`);
  });
}
...
function triggerCampaignRefresh(): void {
  const nav = activeNav();
  if (nav !== "campaigns" && nav !== "studio") return;
  sidebar?.querySelector<HTMLButtonElement>(`button[data-nav="${nav}"]`)?.click();
}
...
document.addEventListener("submit", (event) => {
  ...
  if (kind === "product-truth") {
    void productStore.load(workspaceId).then((workspace) => {
      ...
      return productRevision.reviseProductTruth(workspaceId, {
        identity: {
        ...
        capabilities: lines(data.get("capabilities")), limitations: lines(data.get("limitations")), positioning: String(data.get("positioning")),
        alternatives: lines(data.get("alternatives")), differentiation: lines(data.get("differentiation")), pricing: lines(data.get("pricing")), packaging: lines(data.get("packaging")),
        offers: lines(data.get("offers")), callsToAction: lines(data.get("callsToAction")), brandVoice: lines(data.get("brandVoice")), terminology: parseTerminology(data.get("terminology")),
        accessibilityConstraints: lines(data.get("accessibilityConstraints")), updatedBy: String(data.get("editor")), rationale: String(data.get("rationale")),
      });
  ...
  if (kind === "icp" && id) {
    void productStore.load(workspaceId).then((workspace) => {
      ...
      const dimensions = Object.fromEntries(ICP_DIMENSIONS.map((dimension) => [dimension, {
        rating: asRating(data.get(`${dimension}.rating`)), rationale: String(data.get(`${dimension}.rationale`)),
        evidenceIds: hypothesis.dimensions[dimension].evidenceIds, confidence: String(data.get(`${dimension}.confidence`)) as "low" | "medium" | "high",
      }])) as IcpHypothesis["dimensions"];
      return productRevision.reviseIcp(workspaceId, id, {
        editor: String(data.get("editor")), rationale: String(data.get("rationale")), name: String(data.get("name")), summary: String(data.get("summary")),
        roles: rolesFrom(data), dimensions, disqualifiers: lines(data.get("disqualifiers")), antiIcpConditions: lines(data.get("antiIcpConditions")),
        assumptions: lines(data.get("assumptions")), contradictions: lines(data.get("contradictions")), evidenceIds: checked(data, "evidenceIds"),
        confidence: String(data.get("confidence")) as "low" | "medium" | "high", owner: String(data.get("owner")),
        nextValidationAction: String(data.get("nextValidationAction")), changeConditions: lines(data.get("changeConditions")),
      });
    }).then(() => campaignRevision.revalidateProductAuthority(workspaceId)).then(() => {
  ...
  if (kind === "experiment-start" && id && hypothesisId) {
    void productRevision.startExperiment(workspaceId, hypothesisId, id, String(data.get("actor"))).then(() => {
      announce("Validation experiment started"); triggerProductRefresh();
    }).catch((error: unknown) => inlineFailure(form, error));
    return;
  }
  if (kind === "experiment-complete" && id && hypothesisId) {
    void productRevision.completeExperiment(workspaceId, hypothesisId, id, {
      actor: String(data.get("actor")), evidence: lines(data.get("evidence")), summary: String(data.get("summary")), decision: String(data.get("decision")),
    }).then(() => { announce("Validation experiment completed with outcome evidence"); triggerProductRefresh(); }).catch((error: unknown) => inlineFailure(form, error));
    return;
  }
  if (kind === "experiment-cancel" && id && hypothesisId) {
    void productRevision.cancelExperiment(workspaceId, hypothesisId, id, String(data.get("actor")), String(data.get("rationale"))).then(() => {
      announce("Validation experiment cancelled with rationale"); triggerProductRefresh();
    }).catch((error: unknown) => inlineFailure(form, error));
    return;
  }
  ...
  if (kind === "campaign" && id) {
    ...
    void campaignRevision.reviseCampaign(workspaceId, id, {
      editor: String(data.get("editor")), rationale: String(data.get("rationale")), title: String(data.get("title")), objective: String(data.get("objective")),
      primaryOutcome: String(data.get("primaryOutcome")), primaryAudience: String(data.get("primaryAudience")), audienceKind,
      ...(audienceKind === "selected_icp" && icpHypothesisId ? { icpHypothesisId } : {}), problem: String(data.get("problem")), trigger: String(data.get("trigger")),
      offer: String(data.get("offer")), messageHierarchy: lines(data.get("messageHierarchy")), proof: lines(data.get("proof")), claimIds: checked(data, "claimIds"),
      evidenceIds: checked(data, "evidenceIds"), callToAction: String(data.get("callToAction")), channels: checked(data, "channels") as ChannelKind[],
      assetPlan: lines(data.get("assetPlan")), owner: String(data.get("owner")), successMeasures: lines(data.get("successMeasures")), dependencies: lines(data.get("dependencies")),
  ...
  if (kind === "content-brief" && id) {
    void campaignRevision.reviseContentBrief(workspaceId, id, {
      editor: String(data.get("editor")), rationale: String(data.get("rationale")), title: String(data.get("title")), objective: String(data.get("objective")),
      pillars: lines(data.get("pillars")), themes: lines(data.get("themes")), deliverables: lines(data.get("deliverables")), sourceNotes: lines(data.get("sourceNotes")), owner: String(data.get("owner")),
    }).then(() => { announce("Content brief correction saved for named re-review"); triggerCampaignRefresh(); }).catch((error: unknown) => inlineFailure(form, error));
    return;
  }
  if (kind === "variant" && id) {
    void campaignRevision.reviseVariant(workspaceId, id, {
      editor: String(data.get("editor")), rationale: String(data.get("rationale")), body: String(data.get("body")), constraints: lines(data.get("constraints")),
    }).then(() => { announce("Channel variant correction saved for named re-review"); triggerCampaignRefresh(); }).catch((error: unknown) => inlineFailure(form, error));
  }
}, true);
